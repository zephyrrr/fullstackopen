...
bloglistRouter.put('/:id', async (request, response, next) => {
  ...
  const updatedBlog = await Blog.findByIdAndUpdate(request.params.id, blog, { new: true, runValidators: true }).populate('user', { username: 1, name: 1 }).populate('comments', { content: 1 })
...
bloglistRouter.get('/:id/comments', async (request, response, next) => {
...
bloglistRouter.post('/:id/comments', async (request, response, next) => {
  ...
  if (!comment.content) {
    return response.status(400).json({
      error: 'content or url missing'
    })
  }
  const blog = await Blog.findById(request.params.id)
  comment.blog = blog.id
  const savedNote = await comment.save()
  blog.comments = (blog.comments || []).concat(savedNote._id)
  blog.save()
  response.json(savedNote.toJSON())
})
